...
function processReporterOutput(
    server: net.Server,
    run: vscode.TestRun,
    test: vscode.TestItem,
    isDebugMode: boolean,
    testStartDates: WeakMap<vscode.TestItem, number>
): Promise<string> {
    return new Promise<string>((resolve, reject) => {
        ...
        server
            .on("data", function (line: string) {
                ...
                try {
                    data = JSON.parse(line) as TestResult;
                } catch {
                    return;
                }
                switch (data.type) {
...
async function executeTest(
    ...
    if (isDebugMode) {
        ...
        let success = await vscode.debug.startDebugging(vscode.workspace.workspaceFolders![0], debugConfig);
        if (!success) {
            return 'Failed to start R debugging session';
        }
    }
    else {
    ...
    return processReporterOutput(server, run, test, isDebugMode, testStartDates);
}
...
sink(socket, split=TRUE)  # split=TRUE also keeps output on screen
sink(socket, type = "message")      # capture *stderr*
